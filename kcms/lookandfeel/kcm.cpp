...
void KCMLookandFeel::setColors(const QString &scheme, const QString &colorFile)
{
    ...
    foreach (const QString &grp, conf->groupList()) {
        ...
        foreach (const QString &key, cg.keyList())
        {
            cghome.revertToDefault(key);
        }
        cg.copyTo(&cgd, KConfig::Notify);
    }
...
{
    QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + "/kdedefaults/" + filename;
    return KConfig(QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + "/kdedefaults/" + filename);
}
